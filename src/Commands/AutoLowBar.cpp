...
// END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=CONSTRUCTOR
	...
	//AddParallel(new LastDitchDrive(.6, 0, 14)); //DOES NOT WORK
	//AddSequential(new SetTurretCylinder(Turret_Straight));
	AddParallel(new SetArmPosition(c_armLowPosition));
	AddParallel(new FirePrepSequence(Hood_Stow, Turret_Angled, Long_Shot_Angle));
	//AddSequential(new SetHoodPosition( Hood_Stow));
	AddParallel(new AutoDrivePower(.7, 0, true, false, false));
	...
	AddSequential (new WaveWait(1.6));
	//AddParallel(new SetArmPosition(c_armLowPosition)); //For Low Bar Sensing MAYBE LOWER A BIT
	...
	AddParallel(new AutoDrivePower(-.7, 0, true, false, false));
	AddSequential(new DefenseCrossedSmart(PitchStepDecrease, true, false));
	//AddSequential(new WaveWait(2)); //if we slam into the driver station, we slam into the driver station
	AddSequential(new AutoDrivePower(0, 0, false, false, false));
